...
/**
 * @fileOverview CandidateHologram BUG FIX.
 * Fixes: onStatusChange TypeError, stuck loading state, and fallback visibility.
 */
...
export default function CandidateHologram({ 
  ...
  useEffect(() => {
    ...
    // 3. Procedural Fallback Generator (height ~6.5 units)
    const generateFallbackData = () => {
      for (let i = 0; i < PARTICLE_COUNT; i++) {
        ...
        const y = 3.25 * Math.sin(theta) * Math.sin(phi);
        ...
        // Define region masks for fallback
        if (y < -0.8 && y > -2.0 && Math.abs(x) < 1.0 && z > 1.2) {
          masks[i] = 1; // Mouth
        } else if (y > 0.6 && y < 1.4 && Math.abs(x) > 0.4 && Math.abs(x) < 1.6 && z > 1.4) {
          masks[i] = 2; // Eyes
        }
        
        // Start in formation cloud
    ...
    // 4. Background Load real face.glb
    ...
    loader.load(MODEL_PATH, (gltf) => {
      ...
      if (headMesh) {
        ...
        for (let i = 0; i < PARTICLE_COUNT; i++) {
          ...
          // Masks for GLB logic
          if (ty < -1.0 && ty > -2.4 && Math.abs(tx) < 1.2 && tz > 0.8) {
            masks[i] = 1; // Mouth
          } else if (ty > 0.5 && ty < 1.5 && Math.abs(tx) > 0.4 && Math.abs(tx) < 1.6 && tz > 1.0) {
            masks[i] = 2; // Eyes
          } else {
            masks[i] = 0;
          }
        }
        safeStatusChange("REAL_IDENTITY_SYNC");
      }
      setIsLoading(false);
    }, undefined, (err) => {
      console.warn("[Hologram] GLB Load Failed, Keeping Procedural Fallback.", err);
    ...
    const animate = () => {
      ...
      if (pointsRef.current) {
        ...
        // Progress formation
        if (formationRef.current < 1.0) formationRef.current += 0.01;
        
        const speakIntensity = speakingRef.current ? Math.sin(time * 0.012) * 0.15 : 0;
        const blink = Math.sin(time * 0.002) > 0.98 ? 0.05 : 1.0;
        ...
        for (let i = 0; i < PARTICLE_COUNT; i++) {
          ...
          // Sub-region animations
          if (masks[i] === 1) { // Mouth
            ty += (targets[i3 + 1] > -1.5 ? speakIntensity : -speakIntensity);
          } else if (masks[i] === 2) { // Eyes
            ty *= blink;
          }

          // Smooth lerp to target
          const ease = formationRef.current >= 1.0 ? 0.12 : 0.05;
    ...
    >
      ...
      {isLoading && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/40 backdrop-blur-md">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.5em] text-accent animate-pulse">Synchronizing Identity...</p>
        </div>
      )}
    </div>
  );
