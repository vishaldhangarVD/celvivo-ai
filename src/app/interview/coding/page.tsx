...
export default function CodingEnginePage() {
  ...
  const submitCode = async () => {
    ...
    for (let i = 0; i < SUBMIT_STEPS.length; i++) {
      setSubmitStep(i);
      await new Promise(r => setTimeout(r, 900));
    }

    // Final navigation to result page
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-0.5">
              <span className="text-[9px] font-bold text-white/40 uppercase tracking-widest">{journey?.company || "Standard Tech"} • {journey?.role || "Software Engineer"}</span>
          ...
          <Card className="flex-1 glass border-white/5 bg-[#0b0e1a] relative overflow-hidden flex flex-col">
            <div className="h-10 border-b border-white/5 bg-white/[0.02] flex items-center justify-between px-4">
              ...
              <div className="flex items-center gap-4">
                <button disabled={isSubmitting} onClick={() => setCode(selectedLang.starter)} className="text-white/20 hover:text-accent transition-colors disabled:opacity-30"><RotateCcw className="w-3 h-3" /></button>
            ...
            <div className="h-16 border-t border-white/5 bg-white/[0.02] flex items-center justify-between px-6">
              <div className="flex items-center gap-3">
                <Button onClick={runCode} disabled={isRunning || isSubmitting} variant="ghost" className="h-10 px-6 rounded-xl glass border-white/10 text-[10px] font-black uppercase tracking-widest hover:bg-white/5">
                  {isRunning ? <Loader2 className="w-3.5 h-3.5 mr-2 animate-spin" /> : <Play className="w-3.5 h-3.5 mr-2 text-green-400" />}
                  Run Code
                </Button>
                <Button disabled={isSubmitting} variant="ghost" className="h-10 px-6 rounded-xl glass border-white/10 text-[10px] font-black uppercase tracking-widest hover:bg-white/5">
                  <Terminal className="w-3.5 h-3.5 mr-2" /> Custom Input
                </Button>
              </div>
              <Button onClick={submitCode} disabled={isRunning || isSubmitting} className="h-10 px-10 btn-premium rounded-xl text-[10px] font-black uppercase tracking-[0.2em] shadow-[0_10px_40px_rgba(147,51,234,0.3)]">
          ...
          >
            ...
            <div className="space-y-8 max-w-md w-full">
              ...
              <div className="grid gap-3 text-left">
                {SUBMIT_STEPS.map((step, idx) => (
                  ...
                  >
                    ...
                    {submitStep === idx && <Loader2 className="w-4 h-4 animate-spin ml-auto text-accent" />}
                  </motion.div>
                ))}
              </div>
              ...
              <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                <motion.div 
                  initial={{ width: 0 }} 
                  animate={{ width: `${((submitStep + 1) / SUBMIT_STEPS.length) * 100}%` }} 
