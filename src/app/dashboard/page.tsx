...
export default function Dashboard() {
  ...
  const interviewsQuery = useMemo(() => {
    ...
    return query(collection(db, 'users', user.uid, 'interviews'), orderBy('createdAt', 'desc'), limit(10));
  }, [db, user?.uid]);
  ...
  const chartData = useMemo(() => {
    ...
    return [...interviews].reverse().map((item: any, i) => ({
      name: `S${i + 1}`,
  ...
  const averageScore = useMemo(() => {
    ...
    return Math.round(interviews.reduce((acc, curr: any) => acc + (curr.overallScore || 0), 0) / interviews.length);
  }, [interviews]);
    ...
    <div className="min-h-screen bg-[#050816] pb-32">
      ...
      <main className="container mx-auto px-6 pt-32">
        <div className="max-w-7xl mx-auto space-y-12">
          ...
          >
            <div>
              <Badge className="bg-accent/20 text-accent mb-4 border-none px-4 py-1 text-[10px] tracking-widest font-bold uppercase">Operational Level: Elite</Badge>
          ...
          <div className="grid md:grid-cols-4 gap-6">
            {[
              { label: "Neural Score", val: `${averageScore}%`, icon: Trophy, color: "text-yellow-400" },
              { label: "Resume Index", val: resumes?.[0]?.atsScore || 0, icon: FileText, color: "text-purple-400" },
              { label: "Readiness %", val: "84%", icon: Target, color: "text-blue-400" },
              { label: "Total Rounds", val: interviews?.length || 0, icon: Activity, color: "text-accent" }
            ].map((stat, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 20 }}
          ...
          <div className="grid lg:grid-cols-12 gap-12">
            <div className="lg:col-span-8 space-y-12">
              <Card className="premium-card bg-white/[0.01] border-white/5 p-10">
                <CardHeader className="p-0 mb-10 border-b border-white/5 pb-8 flex flex-row items-center justify-between">
                  <div>
                    <CardTitle className="text-2xl font-bold">Neural Progression</CardTitle>
                    <p className="text-xs text-muted-foreground uppercase tracking-widest mt-1">Growth tracking over last 10 sessions</p>
                  </div>
              ...
              <Card className="premium-card bg-white/[0.01] border-white/5 p-10">
                ...
                <CardContent className="p-0 space-y-4">
                  {interviewsLoading ? (
                    ...
                    interviews.slice(0, 5).map((session: any) => (
                      <div key={session.id} className="flex items-center justify-between p-6 glass rounded-[2rem] border-white/5 hover:bg-white/[0.03] transition-all group">
                        <div className="flex items-center gap-6">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-muted-foreground uppercase tracking-widest">{session.experienceLevel} Track</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-12">
                          ...
                          <Link href={`/feedback/${session.id}`}>
                            <Button variant="ghost" size="icon" className="group-hover:text-accent rounded-xl"><ArrowUpRight className="w-5 h-5" /></Button>
                          </Link>
                        </div>
                      </div>
                    ))
                    ...
                    <div className="text-center py-20 border border-dashed border-white/10 rounded-[2rem]">
                      <p className="text-muted-foreground font-light">No sessions recorded yet. Initialize your first arena simulation.</p>
                    </div>
                  )}
                </CardContent>
              </Card>
            ...
            <div className="lg:col-span-4 space-y-8">
              ...
              <Card className="premium-card bg-white/[0.01] border-white/5">
                <CardHeader>
                  <CardTitle className="text-xl font-bold">Neural Gaps</CardTitle>
                </CardHeader>
                <CardContent className="space-y-6">
                  {[
                    { skill: "System Design", gap: 35, color: "bg-red-400" },
                    { skill: "Cloud Architecture", gap: 20, color: "bg-yellow-400" },
                    { skill: "Distributed Logs", gap: 15, color: "bg-blue-400" }
                  ].map((item, i) => (
