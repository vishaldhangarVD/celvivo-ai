          ...
          <Card className="premium-card bg-white/[0.01] border-white/5 p-8">
            <div className="flex flex-col lg:flex-row gap-6">
              ...
              <div className="flex flex-wrap gap-4">
                <select value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)} className="h-14 px-6 glass border-white/10 bg-[#0b0e1a] rounded-2xl text-xs font-bold uppercase tracking-widest outline-none focus:border-accent">
                  <option value="All">All Categories</option>
                  {CATEGORIES.map(cat => <option key={cat} value={cat}>{cat}</option>)}
                </select>
                <select value={selectedCompany} onChange={e => setSelectedCompany(e.target.value)} className="h-14 px-6 glass border-white/10 bg-[#0b0e1a] rounded-2xl text-xs font-bold uppercase tracking-widest outline-none focus:border-accent">
                  <option value="All">All Companies</option>
                  {COMPANIES.map(comp => <option key={comp} value={comp}>{comp}</option>)}
                </select>
                <select value={selectedDifficulty} onChange={e => setSelectedDifficulty(e.target.value as any)} className="h-14 px-6 glass border-white/10 bg-[#0b0e1a] rounded-2xl text-xs font-bold uppercase tracking-widest outline-none focus:border-accent">
                ...
                <Card className="glass rounded-[2rem] border-white/5 hover:border-white/10 transition-all group p-8">
                  <div className="flex flex-col md:flex-row justify-between items-start gap-6 mb-6">
                    <div className="flex flex-wrap gap-3">
                      <Badge className="bg-accent/10 text-accent border-accent/20 font-bold text-[8px] uppercase tracking-widest">{q.category}</Badge>
                      {q.company && <Badge className="bg-purple-500/10 text-purple-400 border-purple-500/20 font-bold text-[8px] uppercase tracking-widest flex items-center gap-1.5"><Building2 className="w-2.5 h-2.5" /> {q.company}</Badge>}
                      <Badge className={`${difficultyColor(q.difficulty)} border-none font-bold text-[8px] uppercase tracking-widest`}>{q.difficulty}</Badge>
                    </div>
                    <div className="flex gap-2">
                      <Button onClick={() => router.push(user ? `/interview/setup?practiceQuestion=${encodeURIComponent(q.text)}` : '/login')} variant="ghost" className="h-10 px-4 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-accent/20 hover:text-accent transition-all flex gap-2"><Play className="w-3.5 h-3.5 fill-current" /> Practice</Button>
                      <Button size="icon" variant="ghost" onClick={() => toggleFavorite(q)} className={`h-10 w-10 rounded-xl transition-all ${favoriteIds.has(q.id) ? 'bg-accent/20 text-accent' : 'hover:bg-white/5 text-white/20'}`}><Star className={`w-5 h-5 ${favoriteIds.has(q.id) ? 'fill-accent' : ''}`} /></Button>
                    </div>
                  </div>
                  <h3 className="text-xl md:text-2xl font-bold leading-tight mb-8 text-white/90">{q.text}</h3>
                  <div className="space-y-4">
                    <Button variant="ghost" onClick={() => toggleReveal(q.id)} className="flex items-center gap-3 text-accent hover:text-accent/80 p-0 h-auto font-bold text-[10px] uppercase tracking-widest group/btn">
                      {revealedIds.has(q.id) ? <><EyeOff className="w-4 h-4" /> Conceal Intelligence</> : <><Eye className="w-4 h-4 group-hover/btn:animate-pulse" /> Reveal Intelligence</>}
                    </Button>
                    <AnimatePresence>
                        ...
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                          <div className="pt-6 border-t border-white/5 mt-4 space-y-6">
                            <div className="glass bg-accent/[0.03] border-accent/10 p-8 rounded-2xl relative">
                              ...
                              <p className="text-lg font-light leading-relaxed text-white/80">{q.answer}</p>
                            </div>
                            <div className="grid md:grid-cols-2 gap-6">
                              <div className="p-6 glass rounded-2xl border-white/5 bg-white/[0.01]">
                                <h5 className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-white/50 mb-4"><Lightbulb className="w-4 h-4 text-yellow-400" /> Interview Tips</h5>
                                <ul className="space-y-3">{(q.tips || []).map((tip, idx) => <li key={idx} className="flex gap-3 text-xs font-light text-white/60 leading-relaxed"><div className="w-1.5 h-1.5 rounded-full bg-accent mt-1.5 shrink-0" />{tip}</li>)}</ul>
                              </div>
                              <div className="p-6 glass rounded-2xl border-white/5 bg-white/[0.01]">
                                <h5 className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-white/50 mb-4"><AlertCircle className="w-4 h-4 text-red-400" /> Common Pitfalls</h5>
                                <ul className="space-y-3">{(q.mistakes || []).map((m, idx) => <li key={idx} className="flex gap-3 text-xs font-light text-white/60 leading-relaxed"><div className="w-1.5 h-1.5 rounded-full bg-red-400 mt-1.5 shrink-0" />{m}</li>)}</ul>
                              </div>
                            </div>
                          </div>
                        </motion.div>
